...
<%
	...
	if (!isVoid(inputStr)) {
		...
		tempElem = YFCDocument.parse("<Location LocationId=\"\"/>").getDocumentElement();
	}

	
%>

	<yfc:callAPI apiName="getLocationDetails" inputElement="<%=inElem%>" 
	templateElement="<%=tempElem%>" outputNamespace=""/>

<%
	
	YFCElement outElem = (YFCElement)request.getAttribute("Location");
