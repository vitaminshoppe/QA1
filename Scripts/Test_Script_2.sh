...
ARCDIR=$LOGDIR/Arch
LOGFILE1=$LOGDIR/BOPS_MonitorOrderResults_Test.txt
...
BOPS_ADDRS="shivraj.ugale@vitaminshoppe.com"
DTC_ADDRS="shivraj.ugale@vitaminshoppe.com"

export LOGDIR ARCDIR PATH ORACLE_HOME USER PWD BOPS_ADDRS DTC_ADDRS TO_ADDRS_ON_FAILURE
...
AND YOH.ORDER_HEADER_KEY > TO_CHAR(SYSDATE - 10,'YYYYMMDD')
...
AND SYSDATE - YORS.STATUS_DATE > '0.015';
exit 0
END
)
...
AND YORS.STATUS_QUANTITY > '0' AND YS.PROCESS_TYPE_KEY = 'ORDER_FULFILLMENT'
AND YORS.STATUS < '1500' AND YORS.STATUS NOT IN ('1300','1310','1400')
AND YOHT.STATUS <> '1300' AND YOHT.HOLD_TYPE NOT IN ('VSI_FRD_REVIEW_HOLD','VSI_FRAUD_HOLD','VSI_REMORSE_HOLD')
AND YOH.PAYMENT_STATUS <> 'AWAIT_PAY_INFO'
AND SYSDATE - YORS.STATUS_DATE > '0.060';
exit 0
END
...
spool $LOGFILE5
ALTER SESSION SET nls_date_format = 'DD-MON-RR HH24:MI:SS'; 
SELECT DISTINCT(TRIM(YOH.ORDER_NO)) AS ORDER_NO, TRIM(YOH.ORDER_DATE) AS ORDER_DATE, TRIM(YS.DESCRIPTION) AS STATUS, TRIM(ROUND(SYSDATE - YORS.STATUS_DATE,3)*24*60) AS AGED_MINS, YOHT.HOLD_TYPE, YOH.PAYMENT_STATUS
FROM STERLING.YFS_ORDER_HEADER YOH, STERLING.YFS_ORDER_RELEASE_STATUS YORS, STERLING.YFS_STATUS YS, STERLING.YFS_ORDER_HOLD_TYPE YOHT
WHERE YOH.ORDER_HEADER_KEY = YORS.ORDER_HEADER_KEY AND YOH.ORDER_HEADER_KEY = YOHT.ORDER_HEADER_KEY AND YORS.STATUS = YS.STATUS
AND YORS.STATUS_QUANTITY > '0' AND YOH.DRAFT_ORDER_FLAG = 'N' AND YS.PROCESS_TYPE_KEY = 'ORDER_FULFILLMENT'
AND YOH.ORDER_HEADER_KEY > TO_CHAR(SYSDATE - 10,'YYYYMMDD') AND YOH.DOCUMENT_TYPE = '0001' AND YOH.ORDER_NO NOT LIKE 'Y1%' AND YOH.EXTN_DTC_ORDER = 'Y' AND YOH.DRAFT_ORDER_FLAG = 'N'
...
then
echo -e "No BOPS orders are waiting to get acknowledged." | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No BOPS orders waiting to be acknowledged.Count:$orderCount1" $BOPS_ADDRS
...
then
echo -e "No DTC orders are stuck in Created status with Remorse Hold.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days" | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No DTC orders are stuck in Created status with Remorse Hold.Count:$orderCount2" $DTC_ADDRS
elif [ $orderCount2 -gt 0 ]
then
echo -e "DTC Orders from OMS PROD stuck in Created status with Remorse Hold.\n\nPlease follow OMS escalation chart if there are more than 5 orders below during WEEKENDS, HOLIDAYS or AFTER OFFICE HOURS.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days.\n\n$order_msg2"|mailx -r "oms_auto_notification@vitaminshoppe.com" -s "DTC Orders stuck in Created status with Remorse Hold. Count:$orderCount2" $DTC_ADDRS
fi
...
then
echo -e "No DTC orders are stuck in Created status with Fraud Hold.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days" | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No DTC orders are stuck in Created status with Fraud Hold.Count:$orderCount3" $DTC_ADDRS
elif [ $orderCount3 -gt 0 ]
then
echo -e "DTC Orders from OMS PROD stuck in Created status with Fraud Hold.\n\nPlease follow OMS escalation chart if there are more than 5 orders below during WEEKENDS, HOLIDAYS or AFTER OFFICE HOURS.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days.\n\n$order_msg3"|mailx -r "oms_auto_notification@vitaminshoppe.com" -s "DTC Orders stuck in Created status with Fraud Hold. Count:$orderCount3" $DTC_ADDRS
fi
...
then
echo -e "No DTC orders are stuck in Created status with Awaiting Payment Info.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days" | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No DTC orders are stuck in Created status with Awaiting Payment Info.Count:$orderCount4" $DTC_ADDRS
...
#echo "${DT}_${TM}"|mailx -s "Error running OMS orders stuck in Created status script" $TO_ADDRS_ON_FAILURE
echo -e "Operators,\n\nPlease check connectivity to OMS servers by logging to App Console 1 and 2. If not able to connect call DB ADMIN.\nDuring weekend and off-hours, also call OMS team (follow OMS escalation).\n\nThanks."|mailx -r "oms_auto_notification@vitaminshoppe.com" -s "Error connecting to OMS Prod DB" $TO_ADDRS_ON_FAILURE
elif [ $orderCount5 -eq 0 ]
then
echo -e "No DTC orders are stuck in Created status.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders aged for more than 10 days" | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No DTC orders are stuck in Created status.Count:$orderCount5" $DTC_ADDRS
