...
then
echo -e "No DTC orders are stuck in Created status with Remorse Hold.\n\nScript Name:Orders_Stuck_Created_Status.sh \nCriteria: Orders older than 80 mins and upto 120 days" | mailx -r "oms_auto_notification@vitaminshoppe.com" -s "No DTC orders are stuck in Created status with Remorse Hold.Count:$orderCount2" $DTC_ADDRS
