...
public class VSINewGetDeliveryLeadTimeUE implements YFSGetDeliveryLeadTimeUE,VSIConstants {
	
	private YFCLogCategory log = YFCLogCategory.instance(VSINewGetDeliveryLeadTimeUE.class);
	private static final String TAG = VSINewGetDeliveryLeadTimeUE.class.getSimpleName();
	YIFApi api;

	@Override
	public Document getDeliveryLeadTime(YFSEnvironment env, Document docInput) throws YFSUserExitException {
		
		log.beginTimer("VSINewGetDeliveryLeadTimeUE.getDeliveryLeadTime : START");
		log.info("VSINewGetDeliveryLeadTimeUE.getDeliveryLeadTime : START");
		...
		//String strCarrierServiceCode= eleGetDeliveryleadTime.getAttribute(VSIConstants.ATTR_CARRIER_SERVICE_CODE);
			...
			//PMR-TS004584867 Changes -- Start
			if(log.isDebugEnabled()){
				log.debug("Input to VSINewGetDeliveryLeadTimeUE API: "+SCXmlUtil.getString(docInput));
			}
			//PMR-TS004584867 Changes -- End
			...
			//PMR-TS004584867 Changes -- Start
			if(log.isDebugEnabled()){
				log.debug("Output to VSINewGetDeliveryLeadTimeUE API: "+SCXmlUtil.getString(docDelLeadTime));
			}
			//PMR-TS004584867 Changes -- End
...
private Document invokeGetDelLeadTimeWebservice(Document docGetDelLeadTimeWebserviceInXML) throws IOException, ParserConfigurationException, SAXException {
		...
		//PMR-TS004584867 Changes -- Start
		if(log.isDebugEnabled()){
			log.debug("Printing input to Get Del Lead Time Webservice XML :"+SCXmlUtil.getString(docGetDelLeadTimeWebserviceInXML));
		}
		//PMR-TS004584867 Changes -- End
