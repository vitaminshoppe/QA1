...
public class VSIESWorkOrderListNew {
	private YFCLogCategory log = YFCLogCategory.instance(VSIESWorkOrderListNew.class);
	YIFApi api;

	public Document vsiESWorkOrderList(YFSEnvironment env, Document inXML) throws Exception {
		...
		// OMS-2878 : Start
		Document vsiESOrderWorkListFinalOutput = XMLUtil.createDocument(VSIConstants.E_ORDER_LIST);
		//OMS-2878 : End
		...
		if (null != ordLine && !VSIUtils.isNullOrEmpty(ordLine.getAttribute("ShipNode"))) {
			...
			//OMS-2964: Start
			ordLine.setAttribute(VSIConstants.ATTR_PIPELINE_KEY, "20140731213919149461");
			//OMS-2964: End
			try {
				...
				/*
				 * Changes done for getting the list of Orders in Restock and appending it to
				 * the OutPut document of above getOrderList
				 */

				// Creating input for getOrderList API for getting the List of orders which are
				// in Restock status
				...
				//OMS-2964: Start
				eleOrderLine.setAttribute(VSIConstants.ATTR_PIPELINE_KEY, "20140731213919149461");
				//OMS-2964: End
				env.setApiTemplate("getOrderList", "/global/template/api/VSIGetOrderList.xml");
				...
				for (int i = 0; i < iOrderLength; i++) {
					...
					for (int j = 0; j < iLineLength; j++) {
						Element OrderLineElement = (Element) OrderElement
								.getElementsByTagName(VSIConstants.ELE_ORDER_LINE).item(j);
						Element LinePriceInfoElement = (Element) OrderElement
								.getElementsByTagName(VSIConstants.ELE_LINE_PRICE).item(j);
						...
						if ((sLineType.equalsIgnoreCase(VSIConstants.LINETYPE_STS))
								|| (sLineType.equalsIgnoreCase(VSIConstants.LINETYPE_PUS))) { // for Line type
																								// SHIP_TO_HOME, no
																								// action required
							String sCustPoNo = OrderLineElement.getAttribute("CustomerPONo"); // getting CustomerPO No
																								// for first line
							String sLineTotal = LinePriceInfoElement.getAttribute(VSIConstants.ATTR_LINE_TOTAL); // getting
																													// Line
																													// total
																													// for
																													// the
							...
							if ((sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_PICKED))
									|| (sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_RESTOCK))
									|| (sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_NOACTION))
									|| (sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_ACKNOWLEDGE))
									|| (sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_AWAIT_CUST))
									|| (sLineStatus.equalsIgnoreCase(VSIConstants.STATUS_STORE_ACK))) { // Check for
																										// Status
								if (!orderStatusKey.containsKey(sCustPoNo)) {
									...
									if (!sLineStatus.equalsIgnoreCase(stLineStatus)) { // Check for partial status in an
																						// Order
					...
					for (String key : orderStatusKey.keySet()) {
						...
						if (!stStatus.equalsIgnoreCase("StatusMismatch")) {
							Element eleOrder = vsiESOrderWorkListOutput.createElement("Order"); // Creating Order
																								// Element
				...
				// OMS-2878 : Start
				//Document vsiESOrderWorkListFinalOutput = XMLUtil.createDocument(VSIConstants.E_ORDER_LIST);
				Element eleOutOrder = vsiESOrderWorkListFinalOutput.getDocumentElement();// Creating OrderListdoc
				NodeList orderNodeList = vsiESOrderWorkListOutput.getElementsByTagName(VSIConstants.ELE_ORDER);
				String Xpath= null;
				int iOrdNodeListLength = orderNodeList.getLength();
				for (int i = 0; i < iOrdNodeListLength; i++) {
					Element orderElement = (Element) orderNodeList.item(i);
					String sCustPoNo = orderElement.getAttribute(VSIConstants.ATTR_CUST_PO_NO);
					if (i == 0) {
						Node nCopy = vsiESOrderWorkListFinalOutput.importNode(orderElement, true);
						vsiESOrderWorkListFinalOutput.getDocumentElement().appendChild(nCopy);
					} else {
						Xpath = "/OrderList/Order[@CustomerPONo='" + sCustPoNo + "']";
						NodeList outOrderList = XMLUtil.getNodeListByXpath(XMLUtil.getDocumentForElement(eleOutOrder),
								Xpath);
						int iSize = outOrderList.getLength();
						if (iSize == 0) {
							Node nCopy = vsiESOrderWorkListFinalOutput.importNode(orderElement, true);
							vsiESOrderWorkListFinalOutput.getDocumentElement().appendChild(nCopy);
			...
			throw new YFSException("EXTN_ERROR", "INVALID_SHIPNODE", "Ship Node cannot be blank");
		}
		//OMS-2878: Start
		if (log.isDebugEnabled()) {
			log.info("================End vsiESWorkOrderList Call================================");
			log.debug("Printing Input XML :" + XmlUtils.getString(vsiESOrderWorkListFinalOutput));
		}
		return vsiESOrderWorkListFinalOutput;
		//OMS-2878 : End
	}
