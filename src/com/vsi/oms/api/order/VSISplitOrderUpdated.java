				...
				if(nInventoryLine.getLength()>0){
					//System.out.println("Try 2");
						...
						//Creating a new line for the quantity which are not available as STS line type
			                ...
			                for (int l = 0; l < nLinecharges; l++) {	
			    				...
			    				String stringCheckBOGO1=chargeName.substring(0, 4);
			    				...
			    				if(stringCheckBOGO1.equalsIgnoreCase("BOGO")){
	                           ...
	                           //** Change for BOP302
			    				...
			    				if(!stringCheckBOGO1.equalsIgnoreCase("BOGO")){
			    					...
			    					*/
									...
									NodeList nSTSlineCharge1=stsOrderLine.getElementsByTagName("LineCharge");
									int stsLineChargeLength1=nSTSlineCharge1.getLength();
									for (int r=0;r<stsLineChargeLength1;r++){
										Element eSTSLineCharge = (Element) nSTSlineCharge1.item(r);
					    				String stsChargeName= eSTSLineCharge.getAttribute("ChargeName");
					    				Element eSTSExtn=(Element) eSTSLineCharge.getElementsByTagName("Extn").item(0);
			    					...
			    					if(isHeaderLevelCharge.equalsIgnoreCase("Y")){
                                        ...
                                        double pickLineAmount=dUnitPrice*(double)availOnHandQty;
                                        double stsLineAmount=dUnitPrice*(double)stsQty;
                                        totalLineamount=pickLineAmount+stsLineAmount;
			                ...
			                for (int l = 0; l < ilineTaxes; l++) {
                               ...
                               dPickTax=Double.parseDouble(df.format(dbTax*pickLinePostChargeamt/totalLineamount));
                               dSTSTax=Double.parseDouble(df.format(dbTax*stskLinePostChargeamt/totalLineamount));
						...
						**/
		    				...
		    				Element eVSICustomerPriceList = XMLUtil.appendChild(inXML, extnEleOrderLine, "VSICustomPriceList", "");
							 ...
							 for (int n = 0; n < dAdjustedqty; n++) {
			    				 ...
			    				 double price=Double.parseDouble(df.format((BOGOLineCharge-roundedOffAmt1)/(double)iteratedAdjustedQty));
			    				 iteratedAdjustedQty--;
			    				 roundedOffAmt1=Double.parseDouble(df.format(price+roundedOffAmt1));
    				...
    				Element eVSICustomerPriceList = XMLUtil.appendChild(inXML, extnEleOrderLine, "VSICustomPriceList", "");
					 ...
					 for (int n = 0; n < dAdjustedqty; n++) {
	    				 ...
	    				 double price=Double.parseDouble(df.format((BOGOLineCharge-roundedOffAmt2)/(double)iteratedAdjustedQty));
	    				 iteratedAdjustedQty--;
	    				 roundedOffAmt2=Double.parseDouble(df.format(price+roundedOffAmt2));
