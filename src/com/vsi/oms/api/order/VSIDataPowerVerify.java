 ...
 * After Save Card(updating date) we have to Verify the CC
 ...
 *
 */
public class VSIDataPowerVerify {
	
	private YFCLogCategory log = YFCLogCategory.instance(VSIDataPowerVerify.class);
	YIFApi api;
	
	public Document vsiDataPower(YFSEnvironment env, Document inXML)
			throws Exception {
		Document returnDoc = null;
		
		if(log.isDebugEnabled()){
			log.info("================Inside VSIDataPowerVerify================================");
			log.debug("Printing Input XML :" + XmlUtils.getString(inXML));
		}
		try{
	
			//System.out.println("Printing Input XML :" + XmlUtils.getString(inXML));	
			String seqName = "VSI_SEQ_TNS_VERIFY";
	    	String sequenceNumber = VSIDBUtil.getNextSequence(env, seqName);
		...
		//System.out.println("Test URL"+TNS_URL);
			//String card = rootEle.getAttribute("card");
			String Orderid= rootEle.getAttribute("OrderNo");
			//System.out.println("OrderID"+Orderid);
			
			String transactionIdStr= Orderid+sequenceNumber+i;
	
			//System.out.println("transactionIdStr"+transactionIdStr);
