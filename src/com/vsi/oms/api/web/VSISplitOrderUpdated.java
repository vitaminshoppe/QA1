...
public class VSISplitOrderUpdated {
	private YFCLogCategory log = YFCLogCategory.instance(VSISplitOrderUpdated.class);
	//Mixed Cart Changes -- Start
	private static final String TAG = VSISplitOrderUpdated.class.getSimpleName();
	...
	throws Exception {
		...
		if(log.isDebugEnabled()){
			log.debug(TAG +" : "+"================Inside VSISplitOrderUpdated Class and splitOrderLines Method================");
			log.debug(TAG +" : "+"Printing Input XML: "+SCXmlUtil.getString(inXML));
		}
		//Mixed Cart Changes -- End
		...
		try{
			...
			for (int i = 0; i < originalLength; i++) {
				...
				if(VSIConstants.LINETYPE_STH.equals(strLineType)){
					if(log.isDebugEnabled()){
						log.debug(TAG +" : "+"SHIP_TO_HOME line so skipping this line");						
					}
					continue;
				}else{
					if(log.isDebugEnabled()){
						log.debug(TAG +" : "+strLineType+" line so it will be processed");						
					}
					//Mixed Cart Changes -- End
	                ...
	                //Element extnEle=(Element)eleOrderLine.getElementsByTagName("Extn").item(0);   //Mixed Cart Changes
				...
				*/
					//Mixed Cart Changes -- Start
					if(log.isDebugEnabled()){						
						log.debug(TAG +" : "+"getAvailableInventory API Input XML: "+SCXmlUtil.getString(createGetAvailInv));
					}
					//Mixed Cart Changes -- End
					...
					//Mixed Cart Changes -- Start
					if(log.isDebugEnabled()){						
						log.debug(TAG +" : "+"getAvailableInventory API Output XML: "+SCXmlUtil.getString(getAvailInvOutDoc));
					}
					//Mixed Cart Changes -- End
					...
					if(nInventoryLine.getLength()>0){
						//System.out.println("Try 2");
							...
							//Mixed Cart Changes -- Start
							if(log.isDebugEnabled()){						
								log.debug(TAG +" : "+"Available ONHAND Quantity is: "+Integer.toString(availOnHandQty));
							}
							//Mixed Cart Changes -- End
					}
							...
							if(availOnHandQty<qty){
								...
								//Mixed Cart Changes -- Start
								if(log.isDebugEnabled()){						
									log.debug(TAG +" : "+"Available ONHAND Quantity is less than Ordered Quantity, so line will be split into BOPUS and BOSTS");
								}
								//Mixed Cart Changes -- End
							...
							**/
								...
								//Mixed Cart Changes -- Start
								if(log.isDebugEnabled()){						
									log.debug(TAG +" : "+"Available ONHAND quantity is greater than or equals the Ordered quantity, hence its a BOPUS line");
								}
								//Mixed Cart Changes -- End
						...
						//Mixed Cart Changes -- Start
						if(log.isDebugEnabled()){						
							log.debug(TAG +" : "+"Available ONHAND quantity is zero, hence the full line will be converted to BOSTS");
						}
						//Mixed Cart Changes -- End						
		...
		//Mixed Cart Changes -- Start
		if(log.isDebugEnabled()){
			log.debug(TAG +" : "+"Printing Output XML: "+SCXmlUtil.getString(inXML));
			log.debug(TAG +" : "+"================Exiting VSISplitOrderUpdated Class and splitOrderLines Method================");			
		}
		//Mixed Cart Changes -- End
		return inXML;
	}
